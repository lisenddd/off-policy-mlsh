...
def rollout(env, agent, N, T, high_len, gamma, lam, test=False, train_sub=False):
    ...
    agent.normalize_adv()
    if not test:
        wandb.log({"reward": total_reward / N, "action": (total_action_sum * high_len) / (total_action_num * high_len)})
    return total_reward / N, (total_action_sum * high_len) / (total_action_num * high_len)
